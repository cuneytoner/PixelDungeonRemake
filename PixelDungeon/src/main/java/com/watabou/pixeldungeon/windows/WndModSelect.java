...
import com.coner.android.util.Unzip;
...
public class WndModSelect extends Window implements DownloadStateListener {
	...
	public WndModSelect() {
		...
		for (int i = 0; i < options.size(); i++) {
			...
			if (!option.equals(ModdingMode.ReMake) && !option.contains("Download")) {
	...
	protected void onDelete(int index) {
		...
		PixelDungeon.activeMod(ModdingMode.ReMake);
		if (getParent() != null) {
			hide();
		}
		Game.scene().add(new WndModDescription(ModdingMode.ReMake));
	}
