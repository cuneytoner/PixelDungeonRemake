...
public class Utils {
	...
	static private Class<?> getR_Field(String field) {
		try {
			return Class.forName("com.coner.pixeldungeon.remake.R$" + field);
	...
	public static String getClassParam(String className, String paramName, String defaultValue, boolean warnIfAbsent) {
		...
		try {
			String Tmp = className + "_" + paramName;
			Field tmpField = strings.getField(Tmp);
			int tmpint = tmpField.getInt(null);
			return Game.getVar(tmpint);
		} catch (NoSuchFieldException e) {
